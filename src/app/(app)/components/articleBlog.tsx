...
const ArticleBlog = ({data}:{data:Blog}) => {
    return (
        <article className="flex flex-col gap-4 py-5 transition-all hover:shadow-md rounded-lg px-4 hover:cursor-pointer">
            <Image className="w-full rounded-md max-w-[300px] max-h-[250px]" src={data.Thumbnail.url} height={data.Thumbnail.height} width={data.Thumbnail.width} alt={data.Thumbnail.filename} />
